...
export default function PromotionPage() {
  ...
  const [detailPromotion, setDetailPromotion] = useState<PromotionDto | null>(
    null,
  );
  const [dishes, setDishes] = useState<DishesDto[]>([]);
  const [restaurants, setRestaurants] = useState<Restaurant[]>([]);
  const pageNumber = toPositiveInt(
    searchParams.get("pageNumber") ?? searchParams.get("pageIndex"),
    1,
  );
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        response.data.message || "Không thể tải danh sách khuyến mãi",
      );
    } catch (error: unknown) {
  ...
  const handlePageChange = (nextPage: number) => {
    const boundedPage = Math.max(
      1,
      Math.min(nextPage, Math.max(1, totalPages)),
    );
  ...
  const handleCreatePromotion = async (
    promotionData: PromotionUpsertPayload,
  ) => {
