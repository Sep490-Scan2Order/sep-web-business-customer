...
export default function DebtPaymentPage() {
  ...
  const overdueText = useMemo(
    () => formatOverdueDuration(debtStartedAt),
    [debtStartedAt],
  );
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      const response = await apiClient.post(
        API.SUBSCRIPTION.CREATE_COMMISSION_FEE_PAYMENT,
      );
      ...
      toast.error(
        response.data?.message || "Không thể tạo giao dịch thanh toán.",
      );
    } catch (error) {
      ...
      toast.error(
        backendMessage || "Có lỗi xảy ra khi tạo giao dịch thanh toán.",
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="max-w-2xl">
              ...
              <h1 className="mt-2 text-3xl font-semibold text-slate-900 md:text-4xl">
                Thanh toán nợ hoa hồng
              </h1>
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Trang này cho phép tenant tạo giao dịch PayOS để thanh toán
                khoản nợ hoa hồng còn tồn đọng.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <div className="rounded-2xl bg-slate-50 p-4">
                  ...
                  <p
                    className={`mt-2 text-lg font-semibold ${tenantInfo?.isSuspended ? "text-rose-600" : "text-emerald-600"}`}
                  >
              ...
              <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800">
                Sau khi tạo giao dịch, bạn sẽ được chuyển sang PayOS. Khi
                webhook callback hoàn tất, hệ thống sẽ tự động xóa công nợ và mở
                khóa nếu tài khoản đang bị đình chỉ.
              </div>
            </div>
          </div>
