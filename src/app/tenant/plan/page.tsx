...
export default function PlanPage() {
  ...
  // Filter
  const filterSubscriptionInfo = subscriptionTenantInfo.filter(info =>
    ...
    (info.currentPlanName && info.currentPlanName.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  // Checkbox logic
  const handleRestaurantInfoCheckboxChange = (restaurantId: number, checked: boolean) => {
    setSelectedIds(prev => {
  ...
  const checkAllCheckBox = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(filterSubscriptionInfo.map(info => info.restaurantId)));
    } else {
      setSelectedIds(new Set());
    }
  };

  const isAllSelected = filterSubscriptionInfo.length > 0 && filterSubscriptionInfo.every(info => selectedIds.has(info.restaurantId));
  const isSomeSelected = filterSubscriptionInfo.some(info => selectedIds.has(info.restaurantId)) && !isAllSelected;
  ...
  const handleActionRenew = (restaurants: SubscriptionTenantInfo[]) => {
    ...
    if (invalid) {
      toast.error("Một số nhà hàng chưa có gói để gia hạn. Vui lòng kiểm tra lại.");
      return;
    }
    
    // Nếu gia hạn 1 nhà hàng, lấy tên gói để hiển thị. Nếu nhiều nhà hàng, set null để Modal tự hiểu là "Nhiều gói"
    if (restaurants.length === 1) {
      const plan = activePlans.find(p => p.id === restaurants[0].currentPlanId) || null;
  ...
  const executeBulkAction = (type: 'CHANGE_PLAN' | 'RENEW') => {
    ...
    const targets = subscriptionTenantInfo.filter(info => selectedIds.has(info.restaurantId));
    if (type === 'CHANGE_PLAN') handleActionChangePlan(targets);
            ...
            {selectedIds.size > 0 ? (
              ...
              <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2">
                ...
                <button 
                  ...
                  className="px-3 py-1.5 bg-emerald-600 text-white rounded-md text-sm font-medium hover:bg-emerald-700 transition"
                >
                  Gia hạn hàng loạt
                </button>
              </div>
              ...
              <div className="flex gap-2">
                <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><Plus className="w-5 h-5 text-gray-700" /></button>
                <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><SlidersHorizontal className="w-5 h-5 text-gray-700" /></button>
                <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowUpDown className="w-5 h-5 text-gray-700" /></button>
              </div>
            )}
          </div>
          ...
          <table className="w-full table-auto border-collapse text-left">
            <thead>
              ...
              <tr>
                ...
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">ID</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Tên nhà hàng</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Địa chỉ</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Tên gói</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Ngày bắt đầu</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Ngày hết hạn</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">Trạng thái</th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 text-center">Hành động</th>
              </tr>
            </thead>
            <tbody>
              {filterSubscriptionInfo.map((info, index) => (
                <tr key={info.restaurantId} className="border-b border-gray-200 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3">
                    <input
                      type="checkbox"
                      className="rounded border-gray-300"
                      checked={selectedIds.has(info.restaurantId)}
                      onChange={(e) => handleRestaurantInfoCheckboxChange(info.restaurantId, e.target.checked)}
                    />
                  </td>
                  <td className="px-4 py-2 text-sm text-gray-600">{index + 1}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">{info.restaurantName}</td>
                  <td className="px-4 py-2 text-sm text-gray-600 truncate max-w-[200px]" title={info.address}>{info.address}</td>
                  <td className="px-4 py-2 text-sm text-gray-600">
                    {info.currentPlanName ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">
                        <span className="h-1.5 w-1.5 rounded-full bg-green-500"></span> {info.currentPlanName}
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800">
                        <span className="h-1.5 w-1.5 rounded-full bg-red-500"></span> Chưa đăng ký
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-2 text-sm text-gray-600">
                    {info.isActive ? (
                      ...
                      <span className="text-xs font-medium text-red-800">Tạm ngưng</span>
                    )}
                  </td>
                  <td className="px-4 py-2 text-sm text-gray-600">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        onClick={() => handleActionChangePlan([info])}
                      >
                      ...
                      <button
                        ...
                        onClick={() => handleActionChangePlan([info])}
                        className="inline-flex items-center rounded-md border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs font-medium text-orange-700 disabled:opacity-50 hover:bg-orange-100"
                      >
                        Hạ cấp
                      </button>
                      <button
                        ...
                        className="inline-flex items-center rounded-md border border-green-200 bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 disabled:opacity-50 hover:bg-green-100"
                      >
                        Gia hạn
                      </button>
                    </div>
