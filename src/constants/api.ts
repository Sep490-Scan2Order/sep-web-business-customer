export const API = {
  ...
  TENANT: {
    ...
    SEND_EMAIL_FORGET_PASSWORD: (email: string) =>
      `/Otp/send-forgot-password?email=${email}`,
    ...
    IS_SUSPENDED: (tenantId: string, isSuspended: boolean) =>
      `/Tenant/IsSuspened?tenantId=${tenantId}&isSuspended=${isSuspended}`,
  },
  ...
  RESTAURANT: {
    ...
    GET_MENU_ALL: (restaurantId: number) =>
      `/Restaurant/${restaurantId}/menu-all`,
    CONFIG_MIN_CASH_AMOUNT: (restaurantId: number, minCashAmount: number) =>
      `/Restaurant/config-min-cash-amount?restaurantId=${restaurantId}&minCashAmount=${minCashAmount}`,
  ...
  CATEGORY: {
    ...
    GET_ALL_BY_TENANT_ID: (tenantId: string) =>
  ...
  DISHES: {
    ...
    UPDATE_DISH: (categoryId: number, dishId: number) =>
      `/Dish/update-dish/${categoryId}/${dishId}`,
    UPDATE_DISH_AVALABILITY: (dishId: number) =>
      `/Dish/update-dish-availability/${dishId}`,
    GET_ALL_BY_TENANT_ID: (tenantId: string) =>
    ...
    DELETE_DISH: (categoryId: number, dishId: number) =>
      `/Dish/delete-dish/${categoryId}/${dishId}`,
  },
  MENU_TEMPLATE: {
    CREATE: "/MenuTemplate",
    GET_ALL: "/MenuTemplate",
    GET_BY_RESTAURANT: (restaurantId: number) =>
      `/MenuTemplate/restaurant/${restaurantId}`,
  ...
  MENU_RESTAURANT: {
    ...
    GET_MENU_BY_RESTAURANT_ID: (restaurantId: number) =>
  ...
  SUBSCRIPTION: {
    ...
    GET_SUBSCRIPTION_PAYMENT_STATUS: (orderCode: number) =>
      `/subscription/payment-status/${orderCode}`,
    CANCEL_PAYMENT: (orderCode: number) =>
      `/subscription/cancel-payment/${orderCode}`,
  ...
  NOTIFICATION: {
    POST: "/Notification",
    GET_ALL: (pageIndex: number = 1, pageSize: number = 7) =>
      `/Notification?pageIndex=${pageIndex}&pageSize=${pageSize}`,
  },
  NOTIFY_TENANT: {
    POST: "/NotifyTenant",
    GET_ALL: "/NotifyTenant",
    COUNT_BY_TENANT_ID: (tenantId: string, notifyTenantStatus: number) =>
      `/NotifyTenant/count/${tenantId}?notifyTenantStatus=${notifyTenantStatus}`,
    UPDATE_READ_BY_TENANT_ID: `/NotifyTenant/update-read-by-tenant`,
    DETAILS: (pageIndex: number = 1, pageSize: number = 5) =>
      `/NotifyTenant/details?pageIndex=${pageIndex}&pageSize=${pageSize}`,
  },
  CONFIGURATION: {
  ...
  ADMIN: {
    ...
    EXPIRING_SUBSCRIPTIONS: (daysThreshold: number = 30) =>
      `/Admin/expiring-subscriptions?daysThreshold=${daysThreshold}`,
