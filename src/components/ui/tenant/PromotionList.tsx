...
export default function PromotionList({
  ...
  const [pendingDeletePromotion, setPendingDeletePromotion] =
  ...
  const getScopeLabel = (promotion: PromotionDto) => {
    return (
      PROMOTION_SCOPE_LABELS[promotion.scope] ?? `Phạm vi ${promotion.scope}`
    );
  };
  ...
  const getAppliedDayLabels = (promotion: PromotionDto) => {
    if (
      promotion.type !== PromotionType.WeeklySpecial ||
      promotion.daysOfWeek <= 0
    ) {
      return [];
    }

    return DAY_OPTIONS.filter(
      (day) => (promotion.daysOfWeek & day.value) !== 0,
    ).map((day) => day.label);
  };
      ...
      {isLoading ? (
        <div className="mt-2 w-full overflow-x-auto">
          <table className="w-full min-w-max table-auto border-collapse text-left">
            <thead>
              <tr>
                <th className="border-b-2 border-gray-200 px-2 py-2 text-sm font-medium text-gray-700 whitespace-nowrap">
                  ID
                </th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 whitespace-nowrap">
                  Tên khuyến mãi
                </th>
                <th className="border-b-2 border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 whitespace-nowrap">
                  Loại
                ...
                <th className="border-b-2 border-gray-200 px-4 py-2 text-center text-sm font-medium text-gray-700">
                  Thao tác
                </th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: 6 }).map((_, rowIndex) => (
                <tr
                  key={`promotion-skeleton-row-${rowIndex}`}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="mx-auto h-16 w-16 animate-pulse rounded bg-slate-200" />
                  </td>
                </tr>
              ))}
            </tbody>
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-sm text-gray-600 whitespace-nowrap">
                      <div className="font-medium text-slate-800">
                        {getDiscountValueLabel(promotion)}
                      </div>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-600 whitespace-nowrap">
                      ...
                      {promotion.type === 1 && (
                        <div className="text-xs text-slate-500">
                          Giờ vàng: {promotion.dailyStartTime} -{" "}
                          {promotion.dailyEndTime}
                        </div>
                      )}
                      {promotion.type === 3 && promotion.daysOfWeek > 0 && (
                        <div className="text-xs text-slate-500">
                          Ngày áp dụng:{" "}
                          {getAppliedDayLabels(promotion).join(", ")}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-2 text-sm text-gray-600 whitespace-nowrap">
