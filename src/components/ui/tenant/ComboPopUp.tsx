...
export default function ComboPopUp({
  ...
  const isSubtotalHigherThanComboPrice =
    comboPriceValue > 0 && totalMoneyReview > comboPriceValue;
  const exceededAmount = isSubtotalHigherThanComboPrice
    ? totalMoneyReview - comboPriceValue
    : 0;

  const getItemByDishId = (dishId: number) =>
    selectedItems.find((item) => item.dishId === dishId);
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Chọn danh mục, nhập thông tin combo và chọn các món thành phần
            </p>
          </div>
        ...
        <div className="border-b border-slate-200 px-6 py-3">
          <div className="flex items-center gap-2 text-xs font-medium">
            <span
              className={`rounded-full px-3 py-1 ${step === 1 ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"}`}
            >
              1. Thông tin combo
            </span>
            <span
              className={`rounded-full px-3 py-1 ${step === 2 ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"}`}
            >
              2. Chọn món
            </span>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    Tổng tiền tạm tính: {totalMoneyReview.toLocaleString()} VND
                  </div>
                  <div className="text-xs text-slate-500">
                    Giá combo: {comboPriceValue.toLocaleString()} VND
                  </div>
                  {isSubtotalHigherThanComboPrice && (
                    <div className="mt-1 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">
                      Cảnh báo: Tổng tạm tính đang cao hơn giá combo{" "}
                      {exceededAmount.toLocaleString()} VND.
                    </div>
                  )}
                </div>
                      ...
                      >
                        <label className="flex cursor-pointer items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500">
                              Giá: {dish.price.toLocaleString()} VND
                            </div>
                          </div>
                        </label>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateItemQuantity(dish.id, e.target.value)
                            }
                            disabled={!selected}
