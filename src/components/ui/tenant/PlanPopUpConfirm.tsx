...
export default function PlanPopUpConfirm({
  onClose,
  selectedPlan,
  targetRestaurants,
}: PlanProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await apiClient.post(
        API.SUBSCRIPTION.CREATE_PAYMENT,
        getPayload(),
      );
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {selectedPlan
                ? `Gói mục tiêu: ${selectedPlan.name}`
                : "Gia hạn gói hiện tại"}
              <span className="ml-1 font-medium">
                ({targetRestaurants.length} nhà hàng)
              </span>
            </p>
          </div>
        ...
        <div className="max-h-[65vh] overflow-y-auto p-6 space-y-5">
          {!previewData ? (
            <>
              {selectedPlan && (
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-sm text-slate-600">
                    <span>
                      Tháng: {selectedPlan.monthlyPrice.toLocaleString("vi-VN")}{" "}
                      VND
                    </span>
                    <span>
                      Năm: {selectedPlan.yearlyPrice.toLocaleString("vi-VN")}{" "}
                      VND
                    </span>
                  </div>
                </div>
              ...
              <div>
                <label className="mb-2 flex items-center gap-2 text-sm font-medium text-slate-700">
                  <ShoppingCart className="h-4 w-4 text-slate-500" /> Số lượng
                  (Tháng/Năm)
                </label>
                <input
                  type="number"
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Ví dụ: 3 tương đương 3 tháng hoặc 3 năm tùy chu kỳ.
                </p>
              </div>
            </>
          ) : (
            <div className="space-y-4">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-3 space-y-3">
                  {previewData.details.map(
                    (
                      detail: PreviewSubscriptionResponse["details"][0],
                      index: number,
                    ) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between">
                            <span>Chu kỳ:</span>
                            <span>
                              {detail.cycle === 1 ? "Theo Tháng" : "Theo Năm"} x{" "}
                              {detail.quantity}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span>Giá gốc:</span>
                            <span>
                              {detail.basePrice.toLocaleString("vi-VN")} VND
                            </span>
                          </div>
                          {detail.balanceConverted > 0 && (
                            <div className="flex justify-between text-orange-600">
                              ...
                              <span>
                                {detail.balanceConverted.toLocaleString(
                                  "vi-VN",
                                )}{" "}
                                VND
                              </span>
                          ...
                          <div className="flex justify-between pt-1 font-semibold text-slate-900">
                            ...
                            <span>
                              {detail.amountToPay.toLocaleString("vi-VN")} VND
                            </span>
                          </div>
                          {detail.message && (
